...
def test_encrypt(mock_randombytes, token):
    ...
    assert (
        output_token == token["raw"]
    ), f"{token['name']} did not produce matching token"
    ...
    assert (
        decrypted["message"] == token["message"]
    ), f"{token['name']} decryption did not produce original message"
    if decrypted["footer"]:
        assert (
            decrypted["footer"] == token["footer"]
        ), f"{token['name']} decryption did not produce original footer"
...
def test_sign(token):
    result = paseto.PasetoV2.sign(token["message"], token["key"], token["footer"])
    assert result == token["raw"], f"{token['name']} did not produce matching token"
    verify = paseto.PasetoV2.verify(result, public_key)
    assert (
        verify["message"] == token["message"]
    ), f"{token['name']} verifycation did not produce original message"
    assert (
        verify["footer"] == token["footer"]
    ), f"{token['name']} verifycation did not produce original footer"
...
def test_skip_validation_on_expired():
    ...
    parsed = paseto.parse(key=public_key, purpose="public", token=token, validate=False)
    assert parsed
